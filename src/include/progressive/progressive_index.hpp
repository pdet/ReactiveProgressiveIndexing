...
class ProgressiveIndex {
  public:
    ProgressiveIndex() : column(nullptr), current_position(0), fullIndex(nullptr), full_scan_time(0), interactivity_threshold(0), delta(0){};

    ProgressiveIndex(double delta, bool isTest)
        : column(nullptr), current_position(0), fullIndex(nullptr), full_scan_time(0), interactivity_threshold(0), delta(delta), isTest(isTest){};
    ProgressiveIndex(double interactivityThreshold, double fullScanTime, bool isTest)
        : column(nullptr), current_position(0), fullIndex(nullptr), full_scan_time(fullScanTime), interactivity_threshold(interactivityThreshold),
          delta(0), isTest(isTest), time_threshold(full_scan_time * interactivity_threshold){};
  ...
  protected:
    ...
    virtual ResultStruct create(std::vector<int64_t>& originalColumn, ssize_t& remaining_swaps, int64_t low, int64_t high, bool scan) = 0;
    //! Refine Phase
