...
class ProgressiveQuicksort : public ProgressiveIndex {
  public:
    ...
    ProgressiveQuicksort(double delta, bool isTest) : tree(nullptr), ProgressiveIndex(delta, isTest) { constants = std::make_unique<CostModel>(); };
  ...
  private:
    ...
    ResultStruct create(std::vector<int64_t>& originalColumn, ssize_t& remaining_swaps, int64_t low, int64_t high, bool scan) override;
