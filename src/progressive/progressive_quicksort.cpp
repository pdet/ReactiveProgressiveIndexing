...
ResultStruct ProgressiveQuicksort::create(vector<int64_t>& originalColumn, ssize_t& remaining_swaps, int64_t low, int64_t high, bool scan) {
    ...
    if (scan) {
        ...
        auto startTimer = system_clock::now();
        ...
        creation_time = duration<double>(endTimer - startTimer).count();
    } else {
        next_index = min(current_position + remaining_swaps, originalColumn.size());
        remaining_swaps -= next_index - current_position;
        for (size_t i = current_position; i < next_index; i++) {
...
double ProgressiveQuicksort::get_cost_for_delta(std::vector<int64_t>& originalColumn, int64_t low, int64_t high, double estimatedDelta) {
    ...
    double page_count =
        (originalColumn.size() / constants->ELEMENTS_PER_PAGE) + (originalColumn.size() % ((int)constants->ELEMENTS_PER_PAGE) != 0 ? 1 : 0);
...
int64_t ProgressiveQuicksort::run_query(std::vector<int64_t>& originalColumn, int64_t low, int64_t high, std::vector<double>& time, size_t query_it) {
    ...
    auto remaining_swaps_aux = remaining_swaps;
    ...
    if (tree->root.get() && tree->noChildren() && !tree->root->sorted) { //! If the node has no children we are still in the creation phase
        ...
        result = create(originalColumn, remaining_swaps, low, high, true);
        ...
        auto startTimerRefine = chrono::system_clock::now();
        refine(remaining_swaps, low, high);
        auto endTimer = system_clock::now();
        creation_time = duration<double>(endTimer - startTimerRefine).count();
    ...
    //! Time to react
    //! FIXME: This could be a while for bad cases
    while (!fullIndex.get() && time[query_it] < 0.95 * time_threshold) {
        startTimer = chrono::system_clock::now();
        auto cost_per_swap = creation_time / (remaining_swaps_aux - remaining_swaps);
        remaining_swaps = (time_threshold - time[query_it]) / cost_per_swap;
        remaining_swaps_aux = remaining_swaps;
        //! we have more to swap
        if (tree->root.get() && tree->noChildren() && !tree->root->sorted) {
            create(originalColumn, remaining_swaps, low, high, false);
        } else if (tree->root.get() && !tree->noChildren()) {
            refine(remaining_swaps, low, high);
        }
